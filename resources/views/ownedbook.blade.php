...
@section('title', 'Book Database | Books You Own')

@php
$obs = new OwnedBookBusinessService();
$obook = $obs->findAllBooks();
@endphp
...
<table class="table table-dark">
    ...
    <tbody>
          @for ($i = 0; $i < count($obook); $i++)
          <tr>
              <td> {{ $obook[$i]['ID'] }} </td>
              <td> {{ $obook[$i]['BOOK_NAME'] }} </td>
              <td> {{ $obook[$i]['AUTHOR'] }} </td>
...
<div id="id01" class="modal">
  ...
  <form class="modal-content" action="add_owned" method="post">
  <input type="hidden" name="_token" value="<?php echo csrf_token() ?>" />

    <div class="container">
      <h1>Add Book</h1>
      <hr>
      <input type="hidden" name = "id" value = "{{ $obook[$i]['ID'] }}">
...
<div id="id02" class="modal">
  ...
  <form class="modal-content" action="edit_owned" method="post">
  <input type="hidden" name="_token" value="<?php echo csrf_token() ?>" />
    <div class="container">
      <h1>Edit Book</h1>
      <hr>
      <input type="hidden" name = "id" value = "{{ $obook[$i]['ID'] }}">
       ...
       <label for="position"><b>Book</b></label>
      <input type="text" placeholder="Enter Book" name="bookName"  value="{{ $obook[$i]['BOOK_NAME'] }} " required>
      <input type="text" placeholder="Enter Author" name="author"  value=" {{ $obook[$i]['AUTHOR'] }} " required>
...
<div id="id07" class="modal">
  ...
  <form class="modal-content" action="delete_owned" method="post">
  <input type="hidden" name="_token" value="<?php echo csrf_token() ?>" />
    <div class="container">
      <h1>Delete Book</h1>
      <input type="hidden" name = "id" value = "{{ $obook[$i]['ID'] }}">
